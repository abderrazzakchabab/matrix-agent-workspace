...
- `specialist_agents`: `id`, workspace, name, model, gateway provider, system policy, tools allowlist, timeout, enabled; no write tool in Phase B.
...
- Modify: `infra/docker-compose.test.yml` to add a Synapse fixture; create `infra/synapse/homeserver.yaml`, `infra/synapse/signing.key`, `tests/fixtures/synapse/seed.sh`
...
- [ ] **Step 4: Verify green and the Synapse fixture.** Run `docker compose -f infra/docker-compose.test.yml up -d postgres synapse && docker compose -f infra/docker-compose.test.yml exec -T synapse /tests/fixtures/synapse/seed.sh`; expected: `seeded @alice:example.test and !room:example.test`. Run the three Vitest files; expected: all tests pass, including `MATRIX_TOKEN_INVALID`, `ROOM_MEMBERSHIP_REQUIRED`, and cross-tenant denial assertions.
- [ ] **Step 5: Commit the focused auth boundary.** Run `git add apps/control-plane/src/auth apps/control-plane/src/app/api/auth apps/control-plane/src/app/api/rooms apps/control-plane/src/db infra/docker-compose.test.yml infra/synapse apps/control-plane/test/auth && git commit -m "feat: authenticate Matrix users and bind rooms explicitly"`.
